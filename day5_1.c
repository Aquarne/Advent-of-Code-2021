...
static void filter_points(struct point_array *array);
...
static void filter_points(struct point_array *points)
{
    struct point_array new;
    point_array_init(&new);
    ...
    for (int i = 0; i < points->count; i += 2)
    {
        ...
        if (p1.x == p2.x || p1.y == p2.y)
        {
            point_array_add(&new, p1);
            point_array_add(&new, p2);
        }
    }
